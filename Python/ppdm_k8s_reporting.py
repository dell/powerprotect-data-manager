...
def get_cluster_api():
    ...
    try:
        ...
        api_server_url = api_server_url.removeprefix("https://").split(":")[0]
...
def get_cluster_api_native():
    ...
    start += len("https://")
    end = cluster.find(":", start)
    if end == -1:
        logger.error("Cannot find port separator in cluster API")
        raise SystemExit("Cannot parse k8s cluster API. Exiting...")

    api_server_url = cluster[start:end]
    logger.info("Detected k8s cluster API: %s", api_server_url)
    return api_server_url
...
def match_cluster_inventory_source(uri, token, cluster, k8s_cluster=None):
    ...
    for reg_cluster in content:
        if (reg_cluster["name"] == k8s_cluster or
                reg_cluster["name"] == cluster):
            return reg_cluster["id"]
        if (reg_cluster["primaryAddress"] == k8s_cluster or
                reg_cluster["primaryAddress"] == cluster):
            return reg_cluster["id"]

    raise SystemExit("Cannot retrieve registered k8s clusters. Exiting...")
...
def get_backup_copies(uri, token, asset_id):
    ...
    copies = response.get("content") if response else None
    if not copies:
...
def main():
    ...
    logger.info("Found %d protected namespaces", len(protected))
    if args.output == "table":
        print(f"-> Protected namespaces: {len(protected)}")
    ...
    report = []
    for asset in assets:
        backup_stats = get_backup_copies(uri_v3, token, asset["id"])
